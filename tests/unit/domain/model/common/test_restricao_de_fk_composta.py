...
def test_restricao_de_fk_composta_guarda_colunas_em_ordem() -> None:
    """Caminho feliz: guarda colunas locais e referenciadas na ordem informada."""
...
def test_restricao_de_fk_composta_aceita_mais_de_duas_colunas() -> None:
    """Caminho feliz: não limita o número de colunas a 2."""
    restricao = RestricaoDeFkComposta(
...
def test_uma_coluna_local_levanta_validation_error() -> None:
    """Erro esperado: 1 coluna não é composta — cabe em `ColunaExtraida.referencia`."""
...
def test_coluna_local_duplicada_levanta_validation_error() -> None:
    """Erro esperado: mesma coluna local repetida não é uma constraint válida."""
...
def test_contagens_diferentes_levanta_validation_error() -> None:
    """Erro esperado: nº de colunas locais e referenciadas precisa ser igual."""
...
def test_restricao_de_fk_composta_e_imutavel() -> None:
    """Borda: RestricaoDeFkComposta é imutável após construção (frozen=True)."""
    restricao = RestricaoDeFkComposta(
        colunas_locais=("a", "b"),
        nome_escopo_referenciado="escopo",
        nome_tabela_referenciada="tabela",
...
def test_restricao_de_fk_composta_permite_colunas_referenciadas_duplicadas() -> None:
    """Borda: colunas referenciadas podem repetir nome (tabela referenciada diferente).
