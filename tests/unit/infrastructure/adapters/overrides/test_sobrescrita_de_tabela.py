...
class TestFeliz:
    ...
    def test_hash_bate_aplica_curadoria_existente(
        self, tmp_path: Path, tabela_extraida: TabelaExtraida
    ) -> None:
        """Hash bate, aplica papel_de_negocio/regras_de_negocio do YAML."""
...
class TestErro:
    ...
    def test_yaml_malformado_retorna_falha(
        ...
        """YAML malformado vira Falha com mensagem clara."""
        caminho = tmp_path / "public" / "clientes.yaml"
...
class TestBorda:
    ...
    def test_arquivo_nao_existe_gera_skeleton_e_aviso(
        ...
        """1ª execução sem YAML existente gera skeleton e emite Aviso de criação."""
        sobrescrita = SobrescritaDeTabela(tmp_path)
    ...
    def test_hash_nao_bate_por_coluna_adicionada_preserva_curadoria(
        ...
        """Coluna nova faz hash não bater; curadoria remanescente é preservada."""
        sobrescrita = SobrescritaDeTabela(tmp_path)
    ...
    def test_hash_nao_bate_sem_mudanca_de_nomes_usa_mensagem_generica(
        ...
        """Hash muda por alteração de tipo (nomes preservados) usa msg genérica."""
        sobrescrita = SobrescritaDeTabela(tmp_path)
        sobrescrita(tabela_extraida)
    ...
    def test_hash_muda_quando_coluna_vira_not_null_ou_unique(
        ...
        """nao_nulavel/unica reais do schema entram no hash estrutural.
    ...
    def test_hash_muda_quando_unique_composto_e_criado(
        ...
        """restricoes_unicas (UNIQUE composto) entra no hash estrutural.
    ...
    def test_hash_muda_quando_fk_composta_e_criada(
        ...
        """restricoes_fk_compostas entra no hash estrutural.
