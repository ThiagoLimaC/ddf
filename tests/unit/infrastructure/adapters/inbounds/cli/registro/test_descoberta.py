...
from tests.unit.infrastructure.adapters.inbounds.cli.registro.fixture_extrator_plugin import (
    REGISTRO_VALIDO as REGISTRO_EXTRATOR_VALIDO,
)
from tests.unit.infrastructure.adapters.inbounds.cli.registro.fixture_gerador_plugin import (
    GeradorPluginFake,
)

from ddf.domain.ports.extrator import ExtratorRegistrado
from ddf.domain.ports.gerador import Gerador
from ddf.infrastructure.adapters.inbounds.cli.registro.descoberta import (
    descobrir_extratores,
    descobrir_geradores,
)
from ddf.infrastructure.adapters.inbounds.cli.registro.extratores import (
...
_MODULO_FIXTURE_EXTRATOR = (
    "tests.unit.infrastructure.adapters.inbounds.cli.registro.fixture_extrator_plugin"
)
_MODULO_FIXTURE_GERADOR = (
    "tests.unit.infrastructure.adapters.inbounds.cli.registro.fixture_gerador_plugin"
)
...
class TestFeliz:
    ...
    def test_descobrir_extratores_registra_plugin_valido_em_registro_isolado(
        self,
    ) -> None:
        """Entry point válido é descoberto e populado no registro isolado."""
        ponto = EntryPoint(
            name="Fake",
    ...
    def test_descobrir_geradores_registra_plugin_valido_em_registro_isolado(
        self,
    ) -> None:
        """Entry point válido é descoberto e populado no registro isolado."""
        ponto = EntryPoint(
...
class TestErro:
    ...
    def test_descobrir_extratores_isola_import_quebrado_e_classe_fora_do_protocol(
        self,
    ) -> None:
        """Import quebrado e classe fora do Protocol viram Aviso isolado."""
    ...
    def test_descobrir_geradores_isola_import_quebrado_e_classe_fora_do_protocol(
        self,
    ) -> None:
        """Import quebrado e classe fora do Protocol viram Aviso isolado."""
        ponto_quebrado = EntryPoint(
...
class TestBorda:
    ...
    def test_descobrir_extratores_com_nomes_duplicados_isola_colisao(
        self,
    ) -> None:
        """Dois entry points com o mesmo nome — o segundo vira Aviso de colisão."""
        ponto_1 = EntryPoint(
            name="Fake",
            value=f"{_MODULO_FIXTURE_EXTRATOR}:REGISTRO_VALIDO",
            group="ddf.extratores",
    ...
    def test_descobrir_geradores_com_nomes_duplicados_isola_colisao(
        self,
    ) -> None:
        """Dois entry points com o mesmo nome — o segundo vira Aviso de colisão."""
        ponto_1 = EntryPoint(
