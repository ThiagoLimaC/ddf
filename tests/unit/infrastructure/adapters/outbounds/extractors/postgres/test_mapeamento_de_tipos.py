...
class TestFeliz:
    ...
    def test_mapeia_varchar_para_varchar(
        self,
    ) -> None:
        """udt_name varchar vira VARCHAR com tamanho_maximo."""
    ...
    def test_mapeia_bpchar_para_char(
        self,
    ) -> None:
        """udt_name bpchar (character(n)) vira CHAR com tamanho_fixo."""
        tipo = mapear_tipo_postgres("bpchar", tamanho_maximo=10)

        assert tipo.categoria == CategoriaDeDado.CHAR
        assert tipo.tamanho_fixo == 10

    def test_mapeia_text_para_text(
        self,
    ) -> None:
        """Text vira TEXT sem atributos."""
        assert mapear_tipo_postgres("text").categoria == CategoriaDeDado.TEXT

    def test_mapeia_numeric_para_numeric_com_precisao_e_escala(
        self,
    ) -> None:
        """numeric(p,s) vira NUMERIC com precisao e escala."""
    ...
    def test_mapeia_int2_e_int4_para_integer(
        self,
    ) -> None:
        """udt_name int2 (smallint) e int4 (integer) viram INTEGER."""
    ...
    def test_mapeia_int8_para_bigint(
        self,
    ) -> None:
        """udt_name int8 (bigint) vira BIGINT."""
        assert mapear_tipo_postgres("int8").categoria == CategoriaDeDado.BIGINT

    def test_mapeia_float4_e_float8_com_precisao_dupla(
        self,
    ) -> None:
        """float4/float8 (real/double precision) compartilham FLOAT."""
    ...
    def test_mapeia_bool_para_boolean(
        self,
    ) -> None:
        """udt_name bool vira BOOLEAN."""
        assert mapear_tipo_postgres("bool").categoria == CategoriaDeDado.BOOLEAN

    def test_mapeia_timestamp_com_e_sem_timezone(
        self,
    ) -> None:
        """udt_name timestamp/timestamptz vira TIMESTAMP com_timezone."""
    ...
    def test_mapeia_time_com_e_sem_timezone(
        self,
    ) -> None:
        """udt_name time/timetz vira TIME com_timezone."""
    ...
    def test_mapeia_json_e_jsonb_para_json(
        self,
    ) -> None:
        """Json e jsonb compartilham a categoria JSON."""
    ...
    def test_mapeia_array_com_elemento_reconhecido(
        self,
    ) -> None:
        """udt_name com prefixo "_" vira ARRAY, elemento sem o "_"."""
...
class TestBorda:
    ...
    def test_tipo_desconhecido_vira_unknown(
        ...
        """udt_name fora da tabela de mapeamento vira UNKNOWN, sem exceção."""
        assert mapear_tipo_postgres("bytea").categoria == CategoriaDeDado.UNKNOWN

    def test_array_com_udt_name_desconhecido_fica_sem_elemento(
        self,
    ) -> None:
        """ARRAY de um tipo sem entrada em _CATEGORIAS_SIMPLES não quebra."""
