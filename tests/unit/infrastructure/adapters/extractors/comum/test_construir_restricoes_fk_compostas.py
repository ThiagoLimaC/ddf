...
class TestFeliz:
    ...
    def test_agrupa_duas_colunas_da_mesma_constraint(
        self,
    ) -> None:
        """2 linhas da mesma constraint viram uma RestricaoDeFkComposta."""
        linhas = [
    ...
    def test_nao_mistura_constraints_diferentes_da_mesma_tabela(
        self,
    ) -> None:
        """2 constraints compostas distintas viram 2 restrições separadas."""
        linhas = [
        ...
        assert {r.nome_tabela_referenciada for r in restricoes} == {
            "estados",
            "periodos",
        }
...
class TestBorda:
    ...
    def test_constraint_de_coluna_unica_e_ignorada(
        self,
    ) -> None:
        """FK de coluna única (grupo de 1) não vira RestricaoDeFkComposta."""
    ...
    def test_lista_de_fks_vazia_retorna_lista_vazia(
        self,
    ) -> None:
        """Nenhuma FK na tabela retorna lista vazia."""
