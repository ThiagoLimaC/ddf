...
class TestFeliz:
    ...
    def test_detecta_email_com_subdominio_e_tld_composto(
        self,
    ) -> None:
        r"""e-mails com subdomínio/TLD composto são detectados.
    ...
    def test_detecta_email_maiusculo(
        self,
    ) -> None:
        """TLD/domínio em maiúsculo também é detectado (re.IGNORECASE)."""
        valores = [f"USER{i}@EMPRESA.COM" for i in range(20)]

        assert detectar_formato(valores) == "email"

    def test_detecta_cnpj(
        self,
    ) -> None:
        """CNPJs formatados são detectados."""
        valores = ["12.345.678/0001-90" for _ in range(20)]

        assert detectar_formato(valores) == "cnpj"

    def test_detecta_cep(
        self,
    ) -> None:
        """CEPs formatados são detectados."""
        valores = ["01310-100" for _ in range(20)]

        assert detectar_formato(valores) == "cep"

    def test_precedencia_quando_valor_bate_mais_de_um_regex(
        self,
    ) -> None:
        """'12345678901' bate cpf e phone; cpf vence por precedência."""
...
class TestBorda:
    ...
    def test_nenhum_formato_detectado_abaixo_do_threshold(
        self,
    ) -> None:
        """só 50% dos valores são e-mail, abaixo do threshold de 80%."""
    ...
    def test_nenhum_formato_detectado_com_menos_de_20_valores(
        self,
    ) -> None:
        """100% de match, mas só 19 valores não atinge o mínimo absoluto."""
        valores = [f"user{i}@empresa.com" for i in range(19)]

        assert detectar_formato(valores) is None

    def test_lista_vazia_nao_detecta_formato(
        self,
    ) -> None:
        """Lista vazia não deve levantar erro, só retornar None."""
