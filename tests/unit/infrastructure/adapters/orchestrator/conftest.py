...
def _tabela_extraida(nome_schema: str, nome_tabela: str) -> TabelaExtraida:
...
class ExtratorFake:
    ...
    def __init__(
        self,
        tabelas_por_schema: dict[str, Resultado[list[tuple[str, str]]]],
        falhas_de_extracao: dict[tuple[str, str], str] | None = None,
    ) -> None:
        """Guarda as respostas fixas de listar_tabelas/extrair_tabela por chave.

        Args:
            tabelas_por_schema: Resultado que listar_tabelas(schema) devolve.
            falhas_de_extracao: mapeia (schema, tabela) -> mensagem de erro;
                ausência da chave significa extração bem-sucedida.
        """
        self._tabelas_por_schema = tabelas_por_schema
        self._falhas_de_extracao = falhas_de_extracao or {}

    def listar_tabelas(self, schema: str) -> Resultado[list[tuple[str, str]]]:
        """Devolve o Resultado pré-configurado para o schema informado."""
        return self._tabelas_por_schema[schema]

    def extrair_tabela(self, schema: str, tabela: str) -> Resultado[TabelaExtraida]:
        """Devolve Falha pré-configurada, ou Sucesso com uma TabelaExtraida mínima."""
        erro = self._falhas_de_extracao.get((schema, tabela))
        if erro is not None:
            return Falha(erro)
        return Sucesso(_tabela_extraida(schema, tabela))
...
class SobrescritaFake:
    ...
    def __init__(self, falhas: dict[tuple[str, str], str] | None = None) -> None:
        """Guarda as falhas pré-configuradas por (schema, tabela).

        Args:
            falhas: mapeia (schema, tabela) -> mensagem de erro; ausência da
    ...
    def __call__(self, entrada: TabelaExtraida) -> Resultado[TabelaCurada]:
        ...
        chave = (entrada.nome_schema, entrada.nome_tabela)
        ...
        return Sucesso(
            TabelaCurada(
                ...
                nome_schema=entrada.nome_schema,
                colunas=colunas,
...
def construir_extrator_fake() -> Callable[..., ExtratorFake]:
    ...
    def _construir(
        ...
        return ExtratorFake(tabelas_por_schema, falhas_de_extracao)
