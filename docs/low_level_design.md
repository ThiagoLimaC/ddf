...
## Paralelismo intra-tabela via `connectorx` (issue #126)

Complementar ao streaming acima — streaming reduz o pico de memória de uma
leitura sequencial longa; isto reduz o tempo de parede dela, lendo faixas
disjuntas da mesma tabela em conexões diferentes. Só ativa pra
`AmostragemIntegral` (tabela inteira) acima do limiar de
`deve_paralelizar_leitura` — `PercentualDeLinhas`/`AmostragemPorFaixa`
continuam sequenciais.

**`deve_paralelizar_leitura`/`reservar_conexoes`/`liberar_conexoes`**
(`extractors/comum/leitura_paralela_intra_tabela.py`, motor-agnóstico,
compartilhado pelos dois Extratores):

```python
def deve_paralelizar_leitura(total_linhas: int, largura_media_bytes: int) -> bool: ...
def reservar_conexoes(semaforo: threading.Semaphore, lock_reserva: threading.Lock, maximo: int, minimo=MINIMO_CONEXOES_PARALELISMO) -> int: ...
...
thread ficar de posse parcial de permits esperando pelo resto (deadlock
hold-and-wait); reduz progressivamente até `minimo`, devolve `0` (cai no
caminho sequencial já existente, sem erro) se nem isso estiver
disponível.

### Por que `connectorx`, não `ThreadPoolExecutor`

Uma primeira implementação reaproveitou o `ThreadPoolExecutor` já usado no
`OrquestradorParalelo` — cada faixa lida numa conexão `psycopg2` própria,
resultado agregado via `pl.DataFrame(orient="row")`. Testada contra uma
tabela real de ~4M linhas: só ~15-20% de ganho de tempo de parede (teto
teórico 4x com 4 conexões). Medição por thread (`time.perf_counter()` vs.
`time.thread_time()`) confirmou o **GIL do Python** como gargalo: a
decodificação de tuplas do driver pra `pl.DataFrame` serializa as threads,
que não liberam o GIL durante esse trabalho — 1.24x de ganho medido com 4
threads. Teto estrutural do desenho, não corrigível por mais paralelismo
dentro dele.

`connectorx` (lib Rust, wheels pré-compiladas, `py.typed` — `mypy --strict`
limpo sem `# type: ignore`) decodifica direto do driver pra Arrow/Polars,
liberando o GIL durante a decodificação (`py.allow_threads`). Spike
validado contra Postgres 16/MariaDB 11 reais, mesma tabela de 4M linhas:
**2.7-4x de ganho real** (9.33s com 4 partições / 6.45s com 8, vs. 25.53s
sequencial) — confirma o GIL como causa raiz, não I/O de disco.

**Trade-off aceito:** `connectorx` não aceita um pool de conexões externo
nem expõe cursor Python — abre e gerencia as próprias conexões, fora do
`ThreadedConnectionPool`/`PooledDB` de cada Extrator. `reservar_conexoes`
continua reservando o orçamento **antes** de chamar `connectorx` (as
conexões que a lib abre por conta própria contam contra o mesmo
`max_conexoes`), mas não há mais cursor Python por faixa pra mockar em
...
`ThreadPoolExecutor` original já validava: a conexão líder
(`_conexao_ja_reservada`, fora do semáforo — o orçamento já foi reservado
por `reservar_conexoes`) abre `SET TRANSACTION ISOLATION LEVEL REPEATABLE
READ` + `SELECT pg_export_snapshot()` e mantém a própria transação aberta
até `connectorx` terminar de ler todas as faixas — é o que sustenta o
snapshot vivo. As `n` faixas de `ctid` (`particoes_de_blocos`,
`postgres/_construcao.py` — determinístico, disjunto, exaustivo, última
faixa sem teto) viram uma lista de strings SQL
(`_query_particao_ctid`, `sql.Composed.as_string`) entregues numa única
chamada:
...
`pre_execution_query` com os dois comandos nessa ordem exata — modo **não
documentado** pela lib, validado empiricamente (sem o `BEGIN` explícito
...
`pg_relation_size`/`ctid` sobre a tabela-mãe não mapeiam pra nada físico
coerente. Cai no caminho sequencial/streaming existente, sem erro.

**Risco aceito, não mitigado nesta rodada:** `NUMERIC` sem precisão/escala
...
tabela sem PK elegível cai no sequencial sem erro, mesmo padrão de tabela
particionada no Postgres.

```python
cx.read_sql(f"mysql://{user}:{password}@{host}:{port}/{escopo}", queries, return_type="polars")
```

`connectorx` declara suporte a MySQL/MariaDB "through mysql protocol", sem
driver dedicado ao dialeto — `ENUM`/`SET` nativos do MariaDB validados
...
tabela — mesma classe de trade-off já aceita pro viés de cluster de
`AmostragemPorFaixa`. `_emitir_aviso_paralelismo_se_necessario` emite um
...
conexões que `connectorx` abre por conta própria (fora do `PooledDB`)
também precisam contar contra o mesmo orçamento `max_conexoes`, senão o
total real de conexões no servidor pode passar do configurado quando o
paralelismo intra-tabela está ativo.

**Fora de escopo desta rodada:** `PercentualDeLinhas`/`AmostragemPorFaixa`
com percentual alto (candidato a tratar como equivalente a
`AmostragemIntegral` pra fins de elegibilidade); detecção de tabela
