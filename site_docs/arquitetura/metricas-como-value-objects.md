...
`TabelaAnalisada`. É a alternativa proibida no `ddf`: um campo novo nesses modelos exige
mudar o modelo, o que exige revisar todo Gerador que já lê `ColunaAnalisada`/
`TabelaAnalisada`, mesmo os que não têm nada a ver com a métrica nova. Uma métrica nova
como Value Object, ao contrário, é um arquivo novo herdando de `MetricaDeColuna` ou
`MetricaDeTabela`: zero mudança em modelo existente, zero Gerador tocado que não precisava
ser.
...
O exemplo mais direto de que essa arquitetura paga o que promete está em
...
o critério à sua própria maneira: os dois leem a mesma métrica, calculada uma única vez,
e aplicam a mesma regra mecânica sobre ela. É o resultado direto de métrica ser Value
Object compartilhável, não campo duplicado por Gerador: a mesma peça de análise estatística
que faz parte do domínio também é o critério de decisão dos artefatos, sem uma segunda
camada de heurística por Gerador.
