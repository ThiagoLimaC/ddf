...
filtro ou agregação: a camada de negócio fica por conta de quem consome esse projeto dbt
a partir daqui, não do `ddf`.
...
que a coluna "parece ser". É aqui que a arquitetura do `ddf` paga o que promete: o mesmo
cálculo estatístico que já é a base do domínio (ver [Analisadores](../guia/analisadores.md))
também é o critério mecânico que decide qual teste de qualidade entra no projeto dbt, sem
uma segunda camada de heurística por cima.
...
- `relationships` é sugerido quando a coluna tem exatamente uma referência de FK própria
  ...
  entra aqui: o teste equivalente, `composite_relationships`, vive no nível do model.
...
tabela. É só uma anotação informativa: não muda a `severity` de nenhum teste sugerido.
